...
function autoFitColumns<T extends Record<string, unknown>>(
	...
	for (const row of rows) {
		headers.forEach((h, i) => {
			...
			if (text.length > widths[i].wch) widths[i].wch = Math.min(60, text.length);
		});
	}
...
export async function buildExportWorkbookBuffer(): Promise<Buffer> {
	const [contacts, activites, labels, natures, events, kanbanColumns] = await Promise.all([
	...
	const contactRows = contacts.map((c) => ({
		...
		KanbanColumnId: (c as { kanbanColumnId?: string | null }).kanbanColumnId ?? "",
	}));
	...
	const eventRows = events.map((e) => ({
		Id: e.id,
		Date:
			e.date instanceof Date ? e.date.toISOString() : new Date(e.date).toISOString(),
		NatureId: e.natureId ?? "",
		Commentaires: e.commentaires ?? "",
		ContactId: e.contactId,
		ContactName: (e as { contact?: { nom?: string | null } }).contact?.nom ?? "",
	}));
