import { ArrowDownWideNarrow, ChevronDown, ArrowUp, ArrowDown } from "lucide-react";
...
export default function SortByDropdown({
	...
	setSortState: Dispatch<SetStateAction<ContactSortMethod>>;
}) {
	return (
		<DropdownMenu>
			...
			<DropdownMenuContent className="min-w-[--radix-dropdown-menu-trigger-width]">
				...
				>
					Nom
					{sortState === ContactSortMethod.NameAsc ? <ArrowUp className="w-4 h-4" /> : null}
					{sortState === ContactSortMethod.NameDesc ? <ArrowDown className="w-4 h-4" /> : null}
				</DropdownMenuItem>
				<DropdownMenuItem
					className="flex justify-between items-center"
					onClick={(event) => {
				...
				>
					Date de rappel
					{sortState === ContactSortMethod.RappelAsc ? <ArrowUp className="w-4 h-4" /> : null}
					{sortState === ContactSortMethod.RappelDesc ? <ArrowDown className="w-4 h-4" /> : null}
				</DropdownMenuItem>
			</DropdownMenuContent>
		</DropdownMenu>
	);
