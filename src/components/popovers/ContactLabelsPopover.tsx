...
export default function ContactLabelsPopover({
	...
	const [selected, setSelected] = useState<Set<string>>(new Set(contact.labels.map((l) => l.id)));
	...
	const toggle = async (labelId: string) => {
		...
		if ("error" in res) {
			...
			contactStore.setContactLabels(contact.id, mapped);
		}
	};
			...
			<PopoverContent className="w-64 p-0">
				...
				<div className="max-h-64 overflow-auto">
					...
					{allLabels && allLabels.length === 0 && (
						<div className="p-3 text-sm text-muted-foreground">Aucun libellé.</div>
					)}
					<ul className="py-1">
						{allLabels?.map((l) => {
