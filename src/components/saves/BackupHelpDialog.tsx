...
type CommandLine = { text: string; prompt?: boolean };
				...
				<div className="p-3 space-y-0.5 leading-relaxed">
					{displayLines.map((line, i) => (
						<div key={i} className="flex gap-2">
							<span className="text-emerald-400 select-none shrink-0 w-3">{line.prompt ? "$" : ""}</span>
			...
			<Dialog open={open} onOpenChange={setOpen}>
				<DialogContent className="max-w-lg">
					<DialogHeader>
						...
						<DialogDescription>
							Lancez une sauvegarde sans ouvrir l&apos;application via un cron ou un appel HTTP.
					...
					<div className="space-y-4">
						...
						<CommandBlock
							...
							lines={[
								{ text: "crontab -e" },
								{ text: "0 2 * * * cd /srv/crm && npm run backup", prompt: true },
							]}
						/>
						<CommandBlock
							title="Appel HTTP"
